...
def evaluateMove(ch, matrix, tup, root, scores):
	...
	words1 = findWords(arr1, len(downRightPath), root)

	arr1 = downLeftPath[::-1]
	arr1.append(ch)
	arr1 = arr1+upRightPath
	words1 += findWords(arr1, len(downLeftPath), root)

	arr1 = downPath[::-1]
	arr1.append(ch)
	arr1 = arr1+upPath
	words1 += findWords(arr1, len(downPath), root)

	arr1 = rightPath[::-1]
	arr1.append(ch)
	arr1 = arr1+leftPath
	words1 += findWords(arr1, len(rightPath), root)
	score = 0
	for word in words1:
...
def bestMove(matrix, tiles, root, scores):
	if densityOfBoard(matrix)>50:
		return bestestMove(matrix, tiles, root, scores)
	else:
		movesSquared=[]
		rowIter = 0
		moves = []
		for row in matrix:
			...
			for col in row:
				if col=='+':
					for tile in tiles:
						evaluation = evaluateMove(tile, matrix, (rowIter, colIter), root, scores)
						# board = deepcopy(matrix)
						# board[rowIter][colIter]=tile
						# enemyScore = dontHelpEnemy(board, tiles, root, scores)
						# summedValue = evaluation - enemyScore
						moves.append((evaluation,rowIter, colIter, tile))
				colIter+=1
			rowIter+=1
		moves.sort()
		moves.reverse()
		maxScore = []
		for move in moves[0:3]:
			board = deepcopy(matrix)
			board[move[1]][move[2]]=tile
			enemyScore = dontHelpEnemy(board, tiles, root, scores)
			maxScore.append((move[0]-enemyScore, move[1],move[2],move[3]))
		# 	score = []
		# 	board = deepcopy(matrix)
		# 	board[move[1]][move[2]]=move[3]
		# 	# nextMoves = maxMove(board, getRidOff(tiles, move[3]), root, scores)
		# 	# for nextMove in nextMoves[0:5]:
		# 	# 	board2 = deepcopy(matrix)
		# 	# 	board2[nextMove[1]][nextMove[2]]=nextMove[3]
		# 	# 	moreMoves = maxMove(board2, getRidOff(tiles, nextMove[3]), root, scores)
		# 	# 	score+=[max(moreMoves)]
		# 	currentScore = max(score)[0]
		# 	enemyScore = dontHelpEnemy(board, tiles, root, scores)
		# 	totalScore = currentScore - enemyScore
		# 	maxScore+=[(totalScore,move[1],move[2], move[3])]

		return max(maxScore)
def getRidOff(arr, ch):
	for el in arr:
		if el==ch:
			del el
	return arr
def maxMove(matrix, tiles, root, scores):
	...
	return moves
	# return max(moves)
def defensiveMove(matrix, tiles,root, scores):
	rowIter = 0
	moves = []
	...
	moves.sort()
	moves.reverse()
	killThisOne = max(moves)
	if killThisOne[3]=='q':
		killThisOne[3]='x'
	else:
		killThisOne[3]='q'
	return killThisOne
def dontHelpEnemy(matrix, tiles, root, scores):
	alph = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z']
	rowIter = 0
	moves = []
	for row in matrix:
...
def bestestMove(matrix, tiles, root, scores):
    counter = 0
    rowIter = 0
    moves = []
    worstMoves = []
        ...
        for col in row:
            if col=='+':
                ...
                for tile in tiles:
                    ...
                    temp[rowIter][colIter] = tile
                    moves.append((evaluateMove(tile, matrix, (rowIter, colIter), root, scores),rowIter, colIter, tile, temp))
            colIter+=1
        rowIter+=1
    ...
    worstMoves.append((first[0] - worstMove(first[-1], ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'], root, scores) * 7/26, first[1], first[2], first[3]))
    worstMoves.append((second[0] - worstMove(second[-1], ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'], root, scores) * 7/26, second[1], second[2], second[3]))
    worstMoves.append((third[0] - worstMove(third[-1], ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'], root, scores) * 7/26, third[1], third[2], third[3]))
    worstMoves.append((fourth[0] - worstMove(third[-1], ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'], root, scores) * 7/26, fourth[1], fourth[2], fourth[3]))
    worstMoves.append((fifth[0] - worstMove(third[-1], ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'], root, scores) * 7/26, fifth[1], fifth[2], fifth[3]))
 
    maxVal = max(worstMoves)
    return maxVal
 
def worstMove(matrix, tiles, root, scores):
    rowIter = 0
    moves = []
    for row in matrix:
        colIter = 0 
        for col in row:
            if col=='+':
                for tile in tiles:
                    val = evaluateMove(tile, matrix, (rowIter, colIter), root, scores)
                    if val != 0:
                        moves.append(val)
            colIter+=1
        rowIter+=1
