	...
	if densityOfBoard(matrix)>50:
		...
		for move in moves[0:5]:
			...
			for nextMove in nextMoves[0:5]:
				...
				# score+=[max(moreMoves)]
				# for moreMove in moreMoves[0:5]:
				# 	board3 = deepcopy(board2)
				# 	deepMoves = maxMove(board3, getRidOff(tiles, moreMove[3]), root, scores)
				value = max(moreMoves)[0]+nextMove[0]+move[0]
				if maxScore[len(maxScore)-1][0]<value:
					maxScore[len(maxScore)-1]=(value,maxScore[len(maxScore)-1][1],maxScore[len(maxScore)-1][2],maxScore[len(maxScore)-1][3])
					# score+=[max(moreMoves)]
			# maxScore+=[(max(score),move[1],move[2], move[3])]
		return max(maxScore)
...
def main():
	...
	if move[0]==0:
		move = defensiveMove(matrix,tiles, root, scores)
